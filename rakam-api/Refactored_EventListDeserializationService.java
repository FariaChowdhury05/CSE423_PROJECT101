...
public class EventListDeserializationService {

    private final JsonEventDeserializer eventDeserializer;
    private final ApiKeyService apiKeyService;

    @Inject
    public EventListDeserializationService(
            ApiKeyService apiKeyService,
            JsonEventDeserializer jsonEventDeserializer) {

        this.eventDeserializer = jsonEventDeserializer;
    ...
    public EventList deserialize(
        ...
        if (fieldName.equals("api")) {
            ...
            InputStream stream =
                    (InputStream) deserializationContext.getAttribute("stream");

            eventsBuffer = jp.readValueAs(TokenBuffer.class);
        } else {
            throw new RakamException(
                    format("Invalid property '%s'", fieldName),
                    BAD_REQUEST
            );
        }
    ...
    private EventList readEvents(
        ...
        Object apiKey =
                deserializationContext.getAttribute("apiKey");
        ...
        if (apiKey == null || apiKey == WRITE_KEY) {
            ...
            if (context == null) {
                throw new RakamException(
                        "api parameter is required",
                        BAD_REQUEST
                );
            }

            try {
                project = apiKeyService.getProjectOfApiKey(
                        context.apiKey,
                        apiKey == null
                                ? WRITE_KEY
                                : (ApiKeyService.AccessKeyType) apiKey
                );
        ...
        for (; t == START_OBJECT; t = jp.nextToken()) {
            list.add(
                    eventDeserializer.deserializeWithProject(
                            jp,
                            project,
                            context,
                            masterKey
        ...
        if (context.checksum != null) {
            Object sourceRef =
                    jp.getTokenLocation().getSourceRef();
    ...
    private void validateChecksum(
        ...
        if (context.apiKey != null) {
            md.update(
                    context.apiKey.getBytes(UTF_8)
            );
        }

        if (context.apiVersion != null) {
            md.update(
                    context.apiVersion.getBytes(UTF_8)
            );
        }

        if (context.uploadTime != null) {
            md.update(
                    String.valueOf(context.uploadTime)
                            .getBytes(UTF_8)
            );
        }
        ...
        md.update(
                sourceRef,
                Ints.checkedCast(start),
                Ints.checkedCast(end - start) + 1
        );
        ...
        if (!md5.equals(
                context.checksum.toUpperCase(Locale.ENGLISH))) {
