...
public class CsvEventDeserializerService {
    ...
    @Inject
    public CsvEventDeserializerService(
            Metastore metastore,
    ...
    public EventList deserialize(
        ...
        boolean useheader =
                Boolean.FALSE != ctxt.getAttribute("useHeader");
        ...
        if (useheader) {
            ...
            List<SchemaField> vall =
                    metastore.getCollection(project, collection);

            header = new AbstractMap.SimpleImmutableEntry<>(
                    vall,
                    IntStream.range(0, vall.size()).toArray()
            );
        }
        ...
        List<FieldType> types = Arrays.stream(indexes)
                .mapToObj(i -> header.getKey().get(i).getType())
                ...
                default:
                    if (idx >= indexes.length) {
                        throw new RakamException(
                                String.format(
                                        "Table has %d columns but csv file has more than %d columns",
                                        indexes.length,
                                        indexes.length
                                ),
        ...
        return new EventList(
                Event.EventContext.apiKey(apiKey),
                project,
                list
        );
    }

    public Map.Entry<List<SchemaField>, int[]> readHeader(
            CsvParser jp,
            String project,
        ...
        while (jp.nextToken() == VALUE_STRING) {
            ...
            String name =
                    ValidationUtil.stripName(
                            jp.getValueAsString(),
                            "header name"
                    );

            Optional<SchemaField> existingField =
                    fields.stream()
            ...
            if (!existingField.isPresent()) {
                ...
                if (name.equals(projectConfig)) {
                    type = configManager.setConfigOnce(
                            project,
                            USER_TYPE.name(),
                            STRING
                    );
                ...
                SchemaField field = dependentFields.values()
                        .stream()
                        .flatMap(Collection::stream)
                        .filter(e -> e.getName().equals(name))
                        .findAny()
                        .orElse(new SchemaField(name, type));
        ...
        if (!newFields.isEmpty()) {
            fields = metastore.getOrCreateCollectionFields(
                    project,
                    collection,
                    schemaChecker.checkNewFields(
                            collection,
                            newFields
                    )
            );
        ...
        int[] indexes = columns.stream()
                .mapToInt(colName ->
                        range(0, finalFields.size())
                                .filter(i ->
                                        finalFields.get(i)
                                                .getName()
                                                .equals(colName))
    ...
    public Object getValue(
        ...
        switch (type) {
            ...
            case TIMESTAMP:
                ...
                try {
                    return DateTimeUtils.parseTimestamp(
                            jp.getValueAsString()
                    );
                } catch (Exception e) {
                    return null;
    ...
    private GenericData.Array getArray(
            FieldType arrayElementType,
            String valueAsString) throws IOException {

        JsonParser parser =
                jsonFactory.createParser(valueAsString);
        ...
        for (; t != JsonToken.END_ARRAY; t = parser.nextToken()) {
            ...
            if (!t.isScalarValue()) {
                throw new JsonMappingException(
                        String.format(
                                "Nested properties are not supported. ('%s' field)",
        ...
        return new GenericData.Array(
                generateAvroSchema(arrayElementType),
                objects
        );
    }
